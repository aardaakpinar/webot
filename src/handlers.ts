...
export async function handleInteraction(
  ...
  if (
    ...
    await interaction.reply(
      "This command can only be used by FAQ admins or the server owner.",
    );
    return;
  }
...
export async function handleMessage(message: Message): Promise<void> {
  ...
  console.log(
    `[${message.server?.name ?? "Unknown"}] FAQ match score: ${(score * 100).toFixed(1)}% for query: ${query}`,
  );
...
async function handleAskFaq(
  interaction: CommandInteraction,
  serverData: ServerData,
): Promise<void> {
  const question =
    interaction.getString("question") ??
  ...
  console.log(
    `[${interaction.server?.name ?? "/list-faqs"}] FAQ match score: ${(score * 100).toFixed(1)}% for query: ${question}`,
  );

  if (!match || score < 0.3) {
    await interaction.reply(
      "No matching answer was found. Try different tags or ask an admin to add one.",
    );
    return;
  }
...
function getInteractionStringOption(
  ...
  const value =
    anyInteraction.getString?.(name) ??
    anyInteraction.getOption?.(name)?.value ??
    anyInteraction.data?.options?.find((opt: any) => opt.name === name)
      ?.value ??
...
async function handleAddFaq(
  ...
  const tagsText =
    interaction.getString("tags") ?? interaction.getString("keywords");
...
async function handleAddAdmin(
  ...
  await interaction.reply(
    `${targetUsername} was added as an FAQ admin.\nServer: ${serverName}`,
  );
}
...
function getUserOptionValue(interaction: CommandInteraction): string | null {
  const value =
    interaction.getOption("user")?.value ??
